...
// func (ca *correlationCalculator) CalculateOneIndex(index string, df *standarlization.LiteralDataFrame) CorrelationResult {
func (ca *correlationCalculator) CalculateOneIndex(index string, table string, df *correlationDF) CorrelationResult {
	// todo:设置ca.data
	df.getValue(index, ca.data)
	...
	if ca.labelNaNRatio > 0.3 || nullRatio(ca.data) > 0.3 {
		...
		if len(ca.data) < 10000 {
			kendallValue = kendallExact(ca.label, ca.data)
		} else {
	...
	result := CorrelationResult{index, table, math.Abs(kendallValue), math.Abs(pearsonValue), math.Abs(spearmanValue)}
	return result
}
